-- Dashboard Materialized Views for PGR KPIs

-- MV 1: Headline KPIs (one row per tenant)
CREATE MATERIALIZED VIEW IF NOT EXISTS pgr_mv_kpi AS
SELECT
  s.tenantid,
  COUNT(*)                     AS total,
...
CREATE UNIQUE INDEX IF NOT EXISTS pgr_mv_kpi_tenantid_idx ON pgr_mv_kpi (tenantid);

-- MV 2: Monthly time series (one row per tenant x month)
CREATE MATERIALIZED VIEW IF NOT EXISTS pgr_mv_monthly AS
SELECT
  s.tenantid,
...
CREATE UNIQUE INDEX IF NOT EXISTS pgr_mv_monthly_tenantid_month_idx ON pgr_mv_monthly (tenantid, month_date);

-- MV 3: Monthly by source (for source line chart)
CREATE MATERIALIZED VIEW IF NOT EXISTS pgr_mv_monthly_source AS
SELECT
  s.tenantid,
  TO_CHAR(TO_TIMESTAMP(s.createdtime / 1000), 'Mon-YYYY')       AS month_label,
  DATE_TRUNC('month', TO_TIMESTAMP(s.createdtime / 1000))::date  AS month_date,
...
CREATE UNIQUE INDEX IF NOT EXISTS pgr_mv_monthly_source_idx ON pgr_mv_monthly_source (tenantid, month_date, source);

-- MV 4: Dimensional breakdown (status/source/type/boundary)
CREATE MATERIALIZED VIEW IF NOT EXISTS pgr_mv_dimension AS
-- By status
...
CREATE UNIQUE INDEX IF NOT EXISTS pgr_mv_dimension_idx ON pgr_mv_dimension (tenantid, dimension, dim_value);
