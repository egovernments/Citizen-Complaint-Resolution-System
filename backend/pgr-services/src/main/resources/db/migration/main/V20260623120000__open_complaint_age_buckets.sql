-- Align open-complaint aging_bucket labels with dashboard histogram buckets:
-- 0-3d, 3-7d, 7-14d, 14d+ (days since filed).
...
SELECT
  ...
  (roll.assignment_count > 0) AS has_been_assigned,
  (roll.assignment_count > 1) AS is_reassigned,
  ...
  CASE WHEN NOT cur.is_open THEN NULL
       WHEN (clock.now_ms - roll.created_at) < 259200000 THEN '0-3d'
       WHEN (clock.now_ms - roll.created_at) < 604800000 THEN '3-7d'
       WHEN (clock.now_ms - roll.created_at) < 1209600000 THEN '7-14d'
       ELSE '14d+' END                                          AS aging_bucket,
