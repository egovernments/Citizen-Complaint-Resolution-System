-- Align PGR analytics with #2048's unified ESCALATE self-loop and cumulative
-- absolute fallback. Flyway migrations are append-only, so recreate both grains:
-- ESCALATE actions must be events even though the retired workflow auto-escalator's
-- boolean flag is false, and complaint facts must use the last reachable absolute
-- rung rather than adding cumulative thresholds.
...
esc AS (
  SELECT tenantid, overrides, default_levels, max_depth
  FROM esc_ranked
