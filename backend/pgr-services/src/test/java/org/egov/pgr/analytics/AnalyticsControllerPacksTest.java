...
public class AnalyticsControllerPacksTest {
    ...
    @BeforeEach
    public void setUp() {
        controller = new AnalyticsController(service, kpiCatalogService, accessService,
                new ObjectMapper(), config);
        when(accessService.resolve(any(), anyString())).thenReturn(AnalyticsAccessFixtures.full());
    ...
    public void matchingPackStillGetsTheTenantCorpusCount() {
        ...
        pack.setRequiredActionUrl(AnalyticsAccess.QUERY);
