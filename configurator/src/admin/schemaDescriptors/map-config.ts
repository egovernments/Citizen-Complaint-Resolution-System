...
export const mapConfigDescriptor: SchemaDescriptor = {
  ...
  fields: [
    ...
    { path: 'geocodeCountryCodes', widget: 'text', label: 'Search country codes',
      help: 'Usually leave blank — worldwide search is fine for most tenants. To restrict address search to specific countries, enter their two-letter ISO codes in lowercase, comma-separated: Kenya = ke, Mozambique = mz, India = in, Uganda = ug. (Full list: search "ISO 3166-1 alpha-2".)' },
    { path: 'searchViewbox.minLon', widget: 'number', label: 'Search box — west (longitude)', min: -180, max: 180, hidden: 'create',
      help: 'You don\'t need to type these — use "Set search area to this view" under the map instead. They are the edges of the box in decimal degrees (longitude −180…180 for west/east, latitude −90…90 for south/north). Addresses outside the box are hidden from search, so a box that is too small hides valid addresses. Clear all four to search the whole country.' },
    { path: 'searchViewbox.minLat', widget: 'number', label: 'Search box — south (latitude)', min: -90, max: 90, hidden: 'create',
      help: 'South edge, latitude in decimal degrees. Set it by framing the map and clicking "Set search area to this view".' },
    { path: 'searchViewbox.maxLon', widget: 'number', label: 'Search box — east (longitude)', min: -180, max: 180, hidden: 'create',
      help: 'East edge, longitude in decimal degrees. Set it by framing the map and clicking "Set search area to this view".' },
    { path: 'searchViewbox.maxLat', widget: 'number', label: 'Search box — north (latitude)', min: -90, max: 90, hidden: 'create',
      help: 'North edge, latitude in decimal degrees. Set it by framing the map and clicking "Set search area to this view".' },
  ],
};
