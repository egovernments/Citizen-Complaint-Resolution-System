    ...
    handler: async (args) => {
      ...
      if (!args.mobile_regex) {
        ...
        try {
          ...
          if (mob && mob.data && mob.data.rules && (mob.data.rules as Record<string, unknown>)['pattern']) {
            (args as any).mobile_regex = (mob.data.rules as Record<string, unknown>)['pattern'];
            const rulesMin = (mob.data.rules as Record<string, unknown>)['minLength'];
            if (!args.mobile_length && rulesMin) {
              (args as any).mobile_length = rulesMin;
            }
          }
        } catch (e) {
      ...
      const validationRules: Array<{ fieldType: string; zone: string; pattern: string; minLength?: number; maxLength?: number; errorMessage?: string }> =
