...
const baseProps = {
  // Multi-select selections (#1455): arrays of {code,...}, not scalar ids.
...
test("a selected ward is named in the bar, and its chip carries the count", () => {
  const html = renderFilters({
    ...baseProps,
    filters: {
      ...baseProps.filters,
  ...
  // The closed multi-select trigger shows "Wards" + a count badge (one chip
  // can stand for several wards); the ward's own label is what the active
  // filter chips below the row spell out.
  assert.match(html, /dashboard-active-filter-chip[\s\S]*Ward One/);
  assert.match(html, /aria-label="Ward filter"[^>]*title="Wards: 1"|title="Wards: 1"[^>]*aria-label="Ward filter"/);
  assert.doesNotMatch(html, /<select/);
});

test("ward chip degrades to a disabled state while options resolve", () => {
  const html = renderFilters({
    ...baseProps,
    filterOptionsLoading: true,
  });
  assert.match(html, /aria-label="Ward filter"[^>]*disabled|disabled[^>]*aria-label="Ward filter"/);
});
