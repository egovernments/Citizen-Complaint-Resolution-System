...
test("the analytics query actions borrow complaint scope by reference, never restate it", () => {
  // A copied scope drifts from PGR search on the first edit, and a top-level "scopeRef": 2008 is
  // read by nobody — the resolver looks for it inside the resource-type block.
  for (const action of unwrap(read(DEFAULT_ACTIONS))) {
    if (![2641, 2642].includes(action.id)) continue;
    assert.deepEqual(action.resource, { complaint: { scopeRef: { actionId: 2008 } } },
      `action ${action.id} must reference action 2008's complaint policy`);
    assert.equal(action.scopeRef, undefined, `action ${action.id} must not carry a top-level scopeRef`);
  }
});
