...
export const UICustomizations = {
  ...
  // SUPERUSER Admin Complaint Search (/employee/pgr/admin-search) — form→param
  // translation + cell renders for AdminSearchConfig (backend: _admin/_search,
  // PR #1260). Departments arrive as multiselect option objects and are sent as
  // repeated departmentCode params ("NA" = the no-department sentinel).
  AdminSearchConfig: {
    preProcess: (data) => {
      const clonedData = _.cloneDeep(data);
      const searchForm = clonedData?.state?.searchForm || {};
      ...
      const rawLimit = Number(clonedData?.state?.tableForm?.limit) || 20;
      const params = {
        tenantId: Digit.ULBService.getCurrentTenantId(),
        // Backend enforces max 50 — clamp here too so the UI never 400s.
      ...
      if (searchForm.complaintNumber) {
        params.serviceRequestId = String(searchForm.complaintNumber).trim().toUpperCase();
      }

      // Date range: toDate is inclusive — send end-of-day.
      const requestDate = searchForm?.range?.requestDate;
      if (requestDate?.startDate && requestDate?.endDate) {
        params.fromDate = new Date(requestDate.startDate).getTime();
        const end = new Date(requestDate.endDate);
        end.setHours(23, 59, 59, 999);
        params.toDate = end.getTime();
      }

      // Departments: multiselect emits [[key, option]...] or option[] depending
      // on the component path — normalise both to a flat code list.
      const rawDepts = filterForm.departments;
      if (Array.isArray(rawDepts) && rawDepts.length > 0) {
        const codes = rawDepts
          .map((d) => (Array.isArray(d) ? d?.[1]?.code : d?.code))
          .filter(Boolean);
        if (codes.length > 0) params.departmentCode = codes;
      }

      // The composer's data hook consumes reqCriteria.params — assign the
      // whole object (same contract as PGRInboxConfig.preProcess).
      clonedData.params = params;
      return clonedData;
    },
    additionalCustomizations: (row, key, column, value, t, searchResult) => {
      const fmtDate = (epoch) => {
        if (!epoch) return t("ES_COMMON_NA");
      ...
      switch (key) {
        ...
        case "ES_PGR_ADMIN_HEADER_DEPARTMENT": {
          if (!value || value === "NA") return <span>{t("ES_PGR_ADMIN_DEPT_NA")}</span>;
          // additionalDetail.department is stored inconsistently: sometimes the
          // MDMS code (EDUCATION_OPS), sometimes the display name. The page
          // caches a code/name → label map when it loads the Department master.
          const deptMap = Digit.SessionStorage.get("ADMIN_SEARCH_DEPT_MAP") || {};
          return <span>{deptMap[value] || value}</span>;
        }

        case "CS_COMPLAINT_DETAILS_COMPLAINT_TYPE": {
          // Same parent-group labeling as the inbox complaint-type line.
          const sc = value;
          const defs = Digit.SessionStorage.get("serviceDefs") || [];
          const def = Array.isArray(defs) ? defs.find((d) => d?.serviceCode === sc) : null;
          const nameByCode = Digit.SessionStorage.get("complaintHierarchyNameByCode") || {};
        ...
        case "ES_PGR_ADMIN_CREATED_DATE":
        case "ES_PGR_ADMIN_LAST_MODIFIED":
    ...
    MobileDetailsOnClick: (row, tenantId) => {
      const complaintNo = row?.["CS_COMMON_COMPLAINT_NO"];
      if (!complaintNo) return `/${window.contextPath}/employee/pgr/admin-search`;
      return `/${window.contextPath}/employee/pgr/complaint-details/${complaintNo}`;
    },
  },
    ...
    additionalCustomizations: (row, key, column, value, t, searchResult) => {
      ...
      switch (key) {
        ...
        case "PROJECT_BOUNDARY_TYPE":
          return value ? <span>{t(`${hierarchyType}_${value?.toUpperCase()}`)}</span> : <span>{t("NA")}</span>;
