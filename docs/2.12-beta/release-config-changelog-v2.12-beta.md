...
- **`RAINMAKER-PGR.MapConfig`** (NEW schema + minimal per-tenant default) — per-tenant map tiles/center/zoom/geocode-bbox config. The default-data-handler seeds `DEFAULT` with neutral tile/ward presentation values; Configurator Phase 2 adds the tenant-specific centre, boundary source and geocoding extent.
