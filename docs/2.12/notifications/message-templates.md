...
| `channel-needs-provider` | An enabled channel has no provider selected, so it falls back to deployment-wide settings. **Error** when routing rows use the channel, a warning otherwise. | Pick a provider on **Notifications → Notification Providers**, Channels card. |
