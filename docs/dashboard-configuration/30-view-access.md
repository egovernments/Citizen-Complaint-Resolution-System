...
- The **route** `/employee/dashboard` is mounted by `AppModules.js` from the registered
  ...
  always registered by **deep link**. Capability-gating is inside `DashboardModule`:
  it calls `/pgr-services/v2/analytics/_access` and fails closed unless
  egov-accesscontrol returns the base dashboard capability (`70` §4).
