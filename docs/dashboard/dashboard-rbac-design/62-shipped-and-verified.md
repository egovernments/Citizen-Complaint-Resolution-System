...
For the operator-facing view of the same system, see `../../2.12/dashboard-configuration.md`.
...
`../../2.12/dashboard-configuration.md`. The relevant RBAC fact: the FE view gate is
...
intended. Full probe: `../../2.12/dashboard-configuration.md` §3.
...
- **`errorLabel` vs `scope_incomplete`** — `KpiTile.errorLabel` maps `pii_forbidden`→"Restricted",
  ...
  **`scope_incomplete`** (`AnalyticsService`), which falls through to the raw code. Still
  ...
  `../../2.12/dashboard-configuration.md` §3.)
