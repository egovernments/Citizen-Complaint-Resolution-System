...
| 62 | **Shipped Realization & Live Verification** | `62-shipped-and-verified.md` | closes the loop: this design **shipped** (esbuild embedding **PR #1062** + the analytics RBAC train) — maps every layer (A–F) to the implemented `file:line` and the **live bomet probe** that proves enforcement (anon→10 PUBLIC, pack first-match, fail-closed scope), and carries the 2 open items forward. Pairs with `../../2.12/dashboard/dashboard-configuration.md`. |
