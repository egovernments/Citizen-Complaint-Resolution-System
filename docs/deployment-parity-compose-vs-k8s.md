...
| 14 | K8s `service-host` map + gateway whitelist carry inherited non-CCRS baggage — hosts for products CCRS doesn't deploy (`.sanitation`×5, `.health`×5, plus FSM/PT/TL/WS/BPA entries) and stale whitelist paths | K8s | Cleanup | One-time prune of `env.yaml` service-host + whitelist down to the services CCRS actually runs. NB: verify the `.es-cluster` `es-client` host separately (with the search-stack review) — that one may be a real wrong-namespace, not just cruft | — | ☐ |
