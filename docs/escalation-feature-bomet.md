# PGR Escalation Feature — Reference Deployment Notes

> **Point-in-time record.** This is the field log of the *first* live escalation
> rollout (the Bomet tenant), kept because the verification recipes and the
> failure modes it caught generalise. It is not maintained in lockstep with the
> feature: for the current behaviour always read
> [`docs/escalation-feature-design.md`](./escalation-feature-design.md), and for
> the current operator procedure read
> [`docs/deploying-escalation-to-new-tenant.md`](./deploying-escalation-to-new-tenant.md).
> Hostnames, IPs and credentials below are placeholders — substitute your own.

This document covers the PGR escalation feature that's been ported into this monorepo across `backend/pgr-services`, `configurator/`, `workflow-designer/` and `tests/integration-tests/`. It mirrors the live state on the Bomet tenant (`<deployment-domain>`) as of the branch `feat/escalation-otel-configurator-designer`.
...
Forked from `<workflow-designer-host>/designer/`. Vanilla React 18 SPA, esbuild bundle, dagre auto-layout, `postMessage` bridge keyed on `<deployment-domain>` / `<other-deployment-domain>` / `localhost`. Built `dist/` ships to `/var/www/.../designer/` on each tenant host.

### `tests/integration-tests/` (this commit)

- **NEW**: `tests/utils/tempo.ts` — small helper to query the per-tenant Tempo (`http://<tempo-host>:13200/api/traces/...`) and assert OTEL span attributes.
...
3. **OTEL custom attributes in Tempo** — confirmed via `http://<tempo-host>:13200/api/traces/<id>`:
   ```
...
## What did NOT yet escalate end-to-end at the time of this log (and why)

> **Since resolved.** The upstream ASSIGN-persistence bug described below
> ([eGovStack/core-services#1674](https://github.com/eGovStack/core-services/issues/1674))
> has been fixed and is live on current deployments; the chain now fires
> end-to-end. See [Assignee-persistence upstream bug](./escalation-feature-design.md#assignee-persistence-upstream-bug)
> in the design doc for the root cause and the fix. The section is kept as the
...
ssh <deployment-host> 'docker logs --tail 50 digit-pgr-services-1 | grep "Escalation skip\|skipBreakdown"'

# Query Tempo for the trace
TRACE_ID=$(ssh <deployment-host> "docker logs --since=30s digit-pgr-services-1 | grep EscalationScheduler | grep -oE 'trace_id=[a-f0-9]{32}' | tail -1 | sed 's/trace_id=//'")
ssh <deployment-host> "curl -s http://<tempo-host>:13200/api/traces/$TRACE_ID" \
  | jq '.batches[].scopeSpans[].spans[] | select(.attributes[]?.key | startswith("escalation."))'

# Open the configurator escalation editor
open https://<deployment-domain>/configurator/

# Open the workflow designer
open https://<deployment-domain>/designer/
```

## Bomet ops layout (informational)

| Path on `<deployment-host>` | Purpose |
|---|---|
...
| `/etc/nginx/sites-available/<deployment-domain>` | `location /designer/` block added idempotently |
