...
| 9 | Role-level escalation (PRD P4) — a complaint sitting in a role inbox (all GROs / LMEs) with no named assignee should escalate to the role's direct supervisor when all members share a department; the multi-department case is explicitly TBD in the PRD itself. Designed opt-in in [`docs/role-escalation-design.md`](./role-escalation-design.md) (panel-reviewed); pending product sign-off + the upstream ASSIGN fix as prerequisite | **Designed — sign-off pending** |
