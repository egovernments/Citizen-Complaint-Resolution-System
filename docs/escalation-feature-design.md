...
> **Implementation status on `develop`**: the docs land ahead of the code. The
> scheduler/service classes exist on `develop`; the CRS SLA schemas, the
> configurator SLA-matrix surface and the role-escalation resolver arrive with
> PR #815. Source links below that point into `configurator/src/resources/crs/`,
> the new `pgr-services` classes or the escalation integration specs resolve
> once that PR merges.
...
- **Run against any deployment.**
  ...
  ```bash
  E2E_BASE_URL=https://<deployment-domain> \
  E2E_USERNAME=ADMIN \
