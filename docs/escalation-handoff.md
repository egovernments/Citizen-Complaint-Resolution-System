...
| Trace-back drawer | `resources/crs/sla-matrix/TraceBackDialog.tsx` | Dry-run `/_trigger` + complaint search for a specific SRID; renders scheduler-verdict / complaint / resolved-SLA panes (5-source cascade) |
...
| **slaSource** | OTEL span attribute: `CRS.CategorySLA.level` / `CRS.CategorySLA` / `CRS.EscalationPolicy.level` / `CRS.StateSLA` / `v0.EscalationConfig`. Which source answered. |
