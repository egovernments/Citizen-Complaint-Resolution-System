...
| E2E role-escalation fixture tenants | `ke.etoeroles` ("E2E Roles Alpha") + `ke.etoebeta` ("E2E Roles Beta") — persistent; hold the fixture-only roles `E2E_SUP1`/`E2E_SUP2`/`E2E_ROLE3`/`E2E_ROLE4` (registered at root `ke`), 8 HRMS employees covering R2 exactly-one / R2 ambiguous / R3 consensus / R3 split / cross-tenant, boundary wards `ETOEROLES_WARD_1`/`ETOEBETA_WARD_1`, per-tenant PGR workflow copies, dual-scoped ADMIN. Safety invariant: zero `E2E_*` holders at production `ke.bomet`. (`ke.e2e*` was impossible — egov-user rejects digits in tenantIds.) | MDMS `tenant.tenants` at `ke` + HRMS/boundary rows on each tenant | rebuild/verify (idempotent): `scp tests/integration-tests/scripts/setup-role-fixture.mjs egov-bomet:/tmp/ && ssh egov-bomet "BASE_URL=http://localhost:18000 node /tmp/setup-role-fixture.mjs"` |
...
| `integration-tests/tests/lifecycle/pgr-escalation-role-flow.spec.ts` | Role-escalation (R1 pin) full-flow E2E: policy snapshot/restore, tuple-scoped 15s SLA + `GRO.ALL` pin, unassigned complaint → dryRun (`R1_PIN` provenance) → real escalation → OTEL child span — *added by `feat/escalation-prd-alignment`* |
| `integration-tests/tests/lifecycle/pgr-escalation-r2r3-flow.spec.ts` | R2 ladder / R3 reportingTo-consensus + ambiguity skips + the cross-tenant memo proof on the persistent fixture tenants `ke.etoeroles`/`ke.etoebeta`; fail-fast concurrent-writer guard on the shared policy row — *added by `feat/escalation-prd-alignment`* |
| `integration-tests/scripts/setup-role-fixture.mjs` | Builds (create-or-verify, idempotent) the persistent role-escalation fixture: tenants `ke.etoeroles`/`ke.etoebeta`, `E2E_*` roles at `ke`, 8 employees, boundaries, workflow copies; `--smoke` files one throwaway complaint per tenant — *added by `feat/escalation-prd-alignment`* |
...
| `escalation-settings-flow.spec.ts` | MUTATING operator-journey e2e: enable role escalation through the real UI (acting role, ladder, max-per-scan) → Save with read-after-write toast → API-assert the exact persisted object → dry-run test scan → pin-lookup limitation UX → disable + byte-identical policy restore (≥120s stability window) — *added by `feat/escalation-prd-alignment`* |
...
- **Role-escalation suites** (`feat/escalation-prd-alignment`) — ⚠ **SERIALIZE these, never run them concurrently** (with each other or with any other escalation suite against the same deployment): all three snapshot/reconfigure the production-shared `CRS.EscalationPolicy` singleton, and the r2r3 spec fails fast with a CONCURRENT WRITER diagnostic when another session touches the row mid-run. Pacing is generous by design (15s tuple SLAs, full 60s breach waits, cron-period sentinels — determinism over speed):
  - **R1 pin full-flow** (~3–9 min, depending on the cron-phase sentinel): `cd tests/integration-tests && BASE_URL=https://bometfeedbackhub.digit.org DIGIT_TENANT=ke.bomet LOCALITY_CODE=BOMET_BOMET_EAST_CHEMANER SERVICE_CODE=ObsoleteOrDamagedPipeline npx playwright test tests/lifecycle/pgr-escalation-role-flow.spec.ts --reporter=line`
  - **R2/R3 + cross-tenant** (~14 min; needs the fixture tenants — rebuild/verify via the §11 cookbook): `cd tests/integration-tests && BASE_URL=https://bometfeedbackhub.digit.org DIGIT_TENANT=ke.bomet LOCALITY_CODE=BOMET_BOMET_EAST_CHEMANER SERVICE_CODE=ObsoleteOrDamagedPipeline npx playwright test tests/lifecycle/pgr-escalation-r2r3-flow.spec.ts --reporter=line`
  - **UI-driven settings flow** (~5 min, including its ≥120s cleanup-stability window that defends against MDMS write redelivery — see §9): `cd configurator && E2E_BASE_URL=https://bometfeedbackhub.digit.org/configurator E2E_TENANT=ke E2E_USERNAME=ADMIN E2E_PASSWORD=eGov@123 npx playwright test --config e2e/playwright.config.ts e2e/escalation-settings-flow.spec.ts --reporter=line`
...
9. **MDMS write redelivery (Kafka → persister)** *(platform behaviour, discovered during e2e cleanup)* — an acked, read-verified MDMS write can be REDELIVERED by the Kafka→`egov-persister` pipeline ~60–80s later and re-applied over a later write. Observed live on the `CRS.EscalationPolicy` singleton: the settings-flow spec's own enable-write reappeared on the row ~60s after a verified restore. "Write then verify once" is NOT a safe restore/cleanup pattern on this stack — anything restoring shared config must re-read until the row stays stable for **≥3 consecutive reads spanning ≥120s** (the UI e2e cleanup does exactly this). Documented in design doc §"MDMS write redelivery (Kafka → persister)". No tracking issue yet.
10. **UI pin look-up can't see city-tenant employees** *(known limitation)* — the Escalation Settings pinned-person editor looks employees up in HRMS at the *page* (state) tenant, so an employee who exists only on a city tenant cannot be looked up — and therefore cannot be pinned — through the UI today. The failure is graceful ("No active employee found…", Save-pin gate stays closed) and is asserted in `configurator/e2e/escalation-settings-flow.spec.ts`; seed such pins via the API/MDMS (`CRS.RoleSupervisors`) until a tenant-scoped look-up ships. No tracking issue yet.
11. **SYSTEM transitions bypass workflow action-role lists** *(upstream question, open)* — `egov-workflow-v2` on current Bomet accepts the escalation cron's SYSTEM-identity `ESCALATE` at states whose action role lists carry only `GRO`/`AUTO_ESCALATE`/`PGR_VIEWER` (no `SYSTEM`); confirmed MUTATING live on the fixture tenants by the r2r3 suite's sentinel — and the behaviour has flipped across Bomet builds (an earlier build rejected the same transition). Consequence: with `roleEscalation` enabled the cron autonomously escalates unattended complaints — intended per the PRD — but whether workflow should honour SYSTEM transitions the state's role list does not grant is an open upstream question. Design doc §"SYSTEM transitions bypass action-role lists". No tracking issue yet.
...
- **Rebuild / verify the role-escalation e2e fixture** — the persistent fixture tenants `ke.etoeroles` / `ke.etoebeta` (used by `pgr-escalation-r2r3-flow.spec.ts` and `escalation-settings-flow.spec.ts`) are built create-or-verify-idempotently by `tests/integration-tests/scripts/setup-role-fixture.mjs`: `scp tests/integration-tests/scripts/setup-role-fixture.mjs egov-bomet:/tmp/ && ssh egov-bomet "BASE_URL=http://localhost:18000 node /tmp/setup-role-fixture.mjs"` — a clean run on an intact fixture is a no-op verify. Add `--smoke` to also file one throwaway PGR complaint per tenant (smoke complaints are creates, not idempotent — leave the flag off for the verify run).
