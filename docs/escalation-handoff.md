...
| Escalation Settings page | `resources/crs/sla-matrix/EscalationSettingsPage.tsx` | `/manage/escalation-settings` — four cards: SLA-source cascade overview (with the status-mapping gate row), escalation-behaviour policy form (`CRS.EscalationPolicy`), complaint-status-mapping editor (`CRS.WorkflowStateMapping`), and a dry-run test-scan card; reads/saves singletons at the **state** tenant with read-after-write verification — *added by `feat/escalation-prd-alignment`* |
