...
- Eligible automatic-escalation states come from `EscalationConfig.eligibleStatuses`; the shipped value covers the LME-assigned state (`PENDINGATLME`) and the GRO-controlled assignment state (`PENDINGFORASSIGNMENT`). The latter is escalated only when that tenant's workflow has put a concrete assignee on it.
...
- `REOPEN` deliberately does not create a new escalation clock or restore consumed rungs: it is the same complaint and remains measured from its original creation. Changing that would require a separate product decision and metadata model.
...
The policy uses one MDMS v2 master, `RAINMAKER-PGR.EscalationConfig`. Its schema is in `utilities/default-data-handler/src/main/resources/schema/RAINMAKER-PGR.json`. The base complaint SLA remains `RAINMAKER-PGR.ComplaintHierarchy.slaHours`. A complete city escalation record overrides the complete state record; fields are not merged. The state record is the shared baseline. Service defaults apply only when neither level returns a record. The checked-in development example is under `utilities/default-data-handler/src/main/resources/mdmsData-dev/`; Nairobi's deployable record is under `ansible/nairobi-mdms/mdms/`; local `full-dump.sql` has no record. The Configurator rejects PGR writes through the competing generic `Workflow.AutoEscalation*` screens. The upgrade migration deactivates existing PGR rows in those masters, and runtime fails closed with `PGR_ESCALATION_CONFIG_CONFLICT` if a PGR row is added directly later; generic workflow escalation remains available to non-PGR products.

If the record is absent or unreadable, pgr-services falls back to `PGR_ESCALATION_ELIGIBLE_STATUSES` / `pgr.escalation.eligible.statuses` (`PENDINGATLME,PENDINGFORASSIGNMENT`), `PGR_ESCALATION_DEFAULT_SLA_MS` / `pgr.escalation.default.sla.ms` (432000000 ms per rung, expanded cumulatively up to the fallback depth), and `PGR_ESCALATION_MAX_DEPTH` / `pgr.escalation.max.depth` (3). `PGR_ESCALATION_ENABLED` / `pgr.escalation.enabled` and `PGR_ESCALATION_INTERVAL_MS` / `pgr.escalation.interval.ms` control only the automatic scheduler; disabling it does not disable manual `ESCALATE`. `PGR_ESCALATION_BATCH_SIZE` / `pgr.escalation.batch.size` is the scheduler page size, not a total-run cap. `PGR_ESCALATION_LOCK_POOL_SIZE` (default 4) and `PGR_ESCALATION_LOCK_CONNECTION_TIMEOUT_MS` (default 30000) bound a private PostgreSQL advisory-lock pool. Locks have no TTL: they cover the whole update, cannot expire mid-call, and PostgreSQL releases them if the process or connection dies. `STATE_LEVEL_TENANT_ID` / `state.level.tenant.id` selects the state root used for tenant discovery. Every configured eligible state must expose an active `ESCALATE` self-loop authorizing `SYSTEM`. Local Compose and local Kubernetes currently set only `PGR_ESCALATION_ENABLED=true`; the remaining values come from `backend/pgr-services/src/main/resources/application.properties` unless an environment overrides them.
...
5. Unassigned complaints—including canonical `PENDINGFORASSIGNMENT` complaints—are skipped before the update pipeline. A tenant workflow may still use that configured state if it assigns there. Top-of-chain complaints remain unchanged with an observable reason.
