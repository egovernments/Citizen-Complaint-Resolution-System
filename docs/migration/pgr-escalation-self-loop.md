...
9. Concurrent scheduler/manual attempts return `ESCALATION_IN_PROGRESS`; the database lease holds no connection across remote calls, releases after a successful synchronous workflow transition, and expires after the configured TTL if a call fails or is interrupted. History/metadata reconciliation checks the next rung on a later attempt.
