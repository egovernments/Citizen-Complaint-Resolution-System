...
> **The short version:** this migration only moves the *master data* that defines complaint categories — it does not touch a single existing complaint. Each category's new `code` is kept **identical** to its old `serviceCode`, so a complaint filed before the migration keeps working exactly as it did before, with no update needed on the complaint itself. **Confirmed in the current v2.12 codebase**: the migration script writes each leaf's `code` as the old `serviceCode` verbatim (`docs/migration/migrate.cjs`, the `code: d.serviceCode` assignment), and `pgr-services` looks up a complaint's category by querying `ComplaintHierarchy` for `code == servicecode` (`PGRConstants.MDMS_SERVICEDEF_SEARCH`, used in `ServiceRequestValidator`) — the same field a pre-migration complaint already has stored. See §1 and §4.1 below for the full detail, and [validation-log-2026-08-24.md](./validation-log-2026-08-24.md) for this confirmed end-to-end against a real deployment.
