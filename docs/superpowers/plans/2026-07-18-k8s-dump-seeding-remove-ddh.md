...
- [x] **Fix `egov-user`'s migration source** (demonstrated live 2026-07-20). NOT a special checksum case: `egov-user-db:master-d69ce29` failed only because it was **built from modified migration files** (a real change to `V20180731215512`). The standard migrations — in-repo at `local-setup/docker/db-migrations/sql/egov-user/`, the same set compose's unified `db-migrations` runs — carry the dump's checksums, so they only trip the *missing*-migration case that the `*:missing` flag already handles. Built `egovio/egov-user-db:dump-parity` from that in-repo source and pinned it as egov-user's `db-migration` init-container in the live k3s deploy with Flyway **ON** → init logged "Successfully validated 31 migrations, up to date, no migration necessary" and the pod went `1/1`. **So with the correct migration source + the `*:missing` flag, ALL 11 services no-op — egov-user included.** For the committed deploy, `egov-user-db` just needs to be built from the standard migrations (not a modified fork); optionally align the app image to compose's `egovio/egov-user:mobilevalidation-jdk8-4984479`. Compose uses **no** `egov-user-db` image — migrations come from the unified `db-migrations` image.
