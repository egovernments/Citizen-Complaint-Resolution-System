...
**Date:** 2026-06-22 **Status:** Draft for review **Version:** 1.0 **Authors:** Engineering team (Mozambique PRD initiative) **Audience:** Engineering team, BRD authors, product owners, IGE/IGSAE IT **Source inputs:**

- BRD V4.0 — `Plataforma de Reclamações e Denúncias` (English translation)  
- Mozambique Engineering Tracker   
...
**Two things blocking ship if not resolved in the next sprint:**
...
| Workflow states | 7 (NEW, IN\_TRIAGE, REFERRED, UNDER\_INVESTIGATION, AWAITING\_INFORMATION, RESOLVED, REJECTED) |
...
4. Notifications (SMS & WhatsApp for citizens, e-mails only for employees at launch; 7 per-state templates)  
5. Complaint Schemas (IGE / IGSAE form variants)  
...
10. Audit logging (state transitions, master data changes, entity changes)
...
4. **Permission role** (`COMPLAINTS_VIEWER`, `COMPLAINTS_EDITOR`, `COMPLAINTS_CREATOR`) — atomic CRUD-style grants on the Complaints entity, independent of the functional role. **Note: Optional to implement but highly useful when some extensibility is needed.**

Scope is not in the functional role name; permission grants are not implied by the functional role — they are separate roles on the user. **A Central Screening Officer's role set, for example, is `{CMS_SCREENING_OFFICER, CENTRAL_USER, COMPLAINTS_VIEWER, COMPLAINTS_EDITOR, EMPLOYEE}`.** Authorization code reads the user's role SET (not a single string), AND-ing the relevant rules together.

**Why this is better?**

- No `department.scope` column, no `user.isCentral` flag changes.   
...
- Defense-in-depth: every state-changing action requires BOTH the functional role (which says "Screening Officer can refer") AND the permission role (`COMPLAINTS_EDITOR` says "this user is allowed to mutate the complaint state at all"). Strip either one and the action is denied.  
...
| 2 | **Reception Officer / Counter Employee** | `{CMS_RECEPTION_OFFICER, CENTRAL_USER, COMPLAINTS_VIEWER, COMPLAINTS_CREATOR, COMPLAINTS_EDITOR, EMPLOYEE}` | Can be mapped to a CENTRAL\_DEPARTMENT  | `CENTRAL_USER` means "no department filter"; intakes land in central pool until screened |
| 3 | **Central Screening Officer** | `{CMS_SCREENING_OFFICER, CENTRAL_USER, COMPLAINTS_VIEWER, COMPLAINTS_EDITOR, EMPLOYEE}` | Can be mapped to a CENTRAL\_DEPARTMENT  | All NEW \+ IN\_TRIAGE cases for the institution |
| 4 | **Department Supervisor** | `{CMS_SUPERVISOR, DEPARTMENT_USER, COMPLAINTS_VIEWER, COMPLAINTS_EDITOR, EMPLOYEE}` | One or more concrete department codes (e.g. `MIN_HEALTH`) | All REFERRED \+ downstream cases in their department(s) |
| 5 | **Case Manager** | `{CMS_CASE_MANAGER, DEPARTMENT_USER, COMPLAINTS_VIEWER, COMPLAINTS_EDITOR, EMPLOYEE}` | one or more concrete department codes | Only cases assigned to *them* within their department(s) |
...
| // Step 1: permission gate (atomic CRUD check) \-- fastest rejectswitch (operation) {    case READ:        if (\!roles.contains("COMPLAINTS\_VIEWER")) deny("no read permission");        break;    case CREATE:        if (\!roles.contains("COMPLAINTS\_CREATOR")) deny("no create permission");        break;    case UPDATE: // includes workflow actions: refer, assign, resolve, reject, requestInfo        if (\!roles.contains("COMPLAINTS\_EDITOR")) deny("no edit permission");        break;}// Step 2: functional gate (state-machine \+ role-to-action matrix)//         e.g. only CMS\_SCREENING\_OFFICER can issue the 'refer' action from IN\_TRIAGEif (\!functionalRoleAllowsAction(roles, currentState, requestedAction)) {    deny("functional role does not permit this action in this state");}// Step 3: scope gate (visibility envelope)boolean isCentral    \= roles.contains("CENTRAL\_USER");boolean isDepartment \= roles.contains("DEPARTMENT\_USER");if (isCentral) {    // institution-wide visibility; no department filter    visibleSet \= allCasesForInstitution(tenantId);} else if (isDepartment) {    // department-scoped visibility per ADR-007    List\<String\> myDepts \= hrmsUserDepartments(user);    visibleSet \= casesWhereDepartmentIn(tenantId, myDepts);} else {    // neither \-- applies to roles like CMS\_ADMIN that don't operate on cases    visibleSet \= none;}// Step 4: targeted-entity check (is the specific case in the visible set?)if (\!visibleSet.contains(targetCaseId)) deny("case not in scope"); |
| :---- |
...
| Inbox / search | Filter by institution (mz.ige or mz.igsae); no department clause | Filter by `applicationStatus IN (REFERRED, …)` AND `department IN HRMS_user.departments` |
...
Existing PGR tenants continue to use legacy role codes (`PGR_LME`, `GRO`, `DGRO`, …); Mozambique deploys with the `CMS_*` \+ scope \+ permission set throughout. 
...
**Note:** There’s too much dependency on a hierarchical state/city construct in the UI, platform and product to remove this safely. This will be tackled as part of a bigger rewrite when we tackle complex tenant configurations.
...
| `ComplaintHierarchy` (schema) | `mz` | sub-tenant | Schema shape (MOZ\_008) is shared |
...
| `LanguageStrategy` (locale precedence \+ fallback) | `mz` | sub-tenant | Per BRD: pt\_MZ default for Mozambique; sub-tenant can pick a different locale strategy |
...
| **egov-hrms** | Existing — remove department mapping from complaint types | Employee accounts with department assignment |
...
└── audit events: STATE\_TRANSITION, PII\_DECRYPT, MASTER\_EDIT, …

    each carries (event\_id, actor, when, before\_state, after\_state, payload\_digest, signature)
...
| :---- | :---- | :---- |
| `RAINMAKER-PGR.ComplaintTemplateType` | `mz, mz.ige + mz.igsae` | extended-attrs design |
| `RAINMAKER-PGR.ComplaintHierarchy` (no department mapping or optional) | `mz.ige` \+ `mz.igsae` | MOZ\_008 \+ ADR-007 |
| `RAINMAKER-PGR.ComplaintRelatedToMap` | `mz` |  |
| `RAINMAKER-PGR.LanguageStrategy` | `mz` | Tenant-specific language strategy and optins |
| Updated `RAINMAKER-PGR.PGRConfiguration` | `mz.ige` \+ `mz.igsae` | MOZ\_007 spec |
...
| WhatsApp (Deferred) | Citizen self-service | No auth — phone number identifies | Conversational; xstate-chatbot |
...
| IN\_TRIAGE | Manual review by Screening Officer | Screening Officer (start triage action) | 24h |
| REFERRED | Assigned to relevant unit \+ Supervisor | Screening Officer (refer action); Supervisor (assignManager self-loop) | 48h |
| UNDER\_INVESTIGATION | Active research by Case Manager | Case Manager (startInvestigation) | Per hierarchy SLA |
| AWAITING\_INFORMATION | Paused — awaiting citizen / third party | Case Manager (requestInformation) | Pauses SLA clock |
...
1. **Central Screening Officer** — role set `{CMS_SCREENING_OFFICER, CENTRAL_USER, EMPLOYEE}` — transitions IN\_TRIAGE → REFERRED, picks a department \+ a Department Supervisor in that department.  
...
- `ComplaintHierarchy` leaves carry `departments[]` (one or more authorised destination departments as optional).  
- HRMS users carry their assigned `departments[]` (Optional: only meaningful when the user has `DEPARTMENT_USER` in their role set).  
- **DEPARTMENT\_USER** in the role set ⇒ intake catalog and inbox are filtered by `HRMS_user.departments[]`  
- **CENTRAL\_USER** in the role set ⇒ intake catalog and inbox span the entire institution; no department filter applied.  
...
SMS, WhatsApp for citizens at launch. E-mail for employees
...
Per Appendix A. Owned by the existing MOZ\_008 work (branch `feat/complaint-classification-hierarchy`).
...
Each leaf carries `name`, `serviceCode`, `slaHours`, `departments[]`, `primaryDepartment`. 
...
| Event type | Trigger |
| :---- | :---- |
| `STATE_TRANSITION` | every workflow action that changes `applicationStatus` |
| `PII_DECRYPT (future)` | every read where `extended_attributes` PII is returned in plain text to a non-citizen role |
| `MASTER_EDIT`  | admin edits to ComplaintHierarchy / TemplateBinding / etc. via configurator |
...
### **ADR-001 — Extend Workflow BusinessService.json to add notification configuration & use PGR as an orchestrator**

**Context.** Notification audience routing today is hardcoded in `pgr-services` (`NotificationService.java:86-116`). The BRD requires per-tenant configurable routing depending on workflow. 

**Decision.** Add an extra configuration to BusinessService.json that PGR can read and render. Egov-workflow-v2 will ignore the extra attributes since it is agnostic to them. This ensures that configuration is in one place. 
...
- *Separate microservice.* Rejected: Too complex for PGR right now. Can be done when we have more use cases.

**Consequences.**

- Workflow changes do not impact applications.  
- PGR owns the business logic of when to trigger notifications.   
...
**Decision.** `ComplaintTemplateType` has exactly two entries: `IGE` and `IGSAE`. The pathway is the discriminator. Category sub-types (Complaint / Grievance / Petition for IGE; economic sectors for IGSAE) are field values driven by the existing `ComplaintHierarchy` (MOZ\_008), not separate templates.
...
### **ADR-007 (OPEN) — Confidentiality scope**
...
| Confidentiality | PII protected per ADR-007 | Per-call masker; audit log on every plaintext decrypt; quarterly PII access review |
| Audit logs | All state transitions \+ master changes \+ entity data changes logged | Emitted to Signed Audit Service; retention managed centrally (7 years per legal basis §2) |
...
| pt\_MZ translation copywriting | Medium | Hari / translator | Allocate translation pass for 30+ SMS bodies \+ UI strings |
...
[Indicative; refined per work item in the linked specs.](https://docs.google.com/spreadsheets/d/1yS4xzWVelfB1feQBGeu614C6VO3KFpXbROWhJFHM_74/edit?usp=sharing)
...
| 8 | SMS template bodies migrate from `egov-localization` to `egov-config-service` | pgr-services (`NotificationUtil` refactor) \+ config-service seed | Both | MOZ\_007 Option B |
...
| 11 | `extended_attributes` JSONB column \+ `ExtendedAttributes` Java model | pgr-services | Product | MOZ\_011 / MOZ\_012 |
| 12 | Schema-driven validation of `extended_attributes` per template | pgr-services validator | Product | MOZ\_012 |
| 13 | **`ComplaintHierarchy` leaf: add `departments[]` array** | MOZ\_008 schema | **Product (NEW — not in current product)** | per ADR-007 |
...
| 17 | Per-tenant feature flags (`useWorkflowExtension`, `useConfigServiceTemplates`) | pgr-services PGRConfiguration | Product (mechanism) \+ Customization (per-tenant values) | per MOZ\_007 |
...
| 21 | 7 SMS templates from BRD §5.1.E (en\_IN \+ pt\_MZ) | egov-config-service data | Customization | needs writing \+ pt\_MZ translation |
...
| 24 | pt\_MZ localization translations (UI strings) | localization data | Customization | net-new copywriting |
...
| 27 | `LanguageStrategy` entry for `mz` (fallback to pt\_MZ) | egov-config-service data | Customization | uses product schema from row 7 |
| 28 | REJECT action requires justification \+ attached document | pgr-services validator | Product | new validation rule |
| 29 | "Request Information" workflow round-trip (AWAITING\_INFORMATION ↔ UNDER\_INVESTIGATION) | pgr-services \+ UI \+ workflow def | Both | new state \+ UI behaviour |
| 30 | Audit emit sites in pgr-services (STATE\_TRANSITION \+ PII\_DECRYPT) into the existing Signed Audit Service | pgr-services | Product | producer wiring only; no new table |
| 31 | Dashboard KPIs per BRD Appendix C (operational \+ executive — two separate surfaces) | dashboard frontend \+ analytics queries | Both | MOZ\_024 |
...
| 37 | Confidentiality flag honoured in all responses (mask PII when `prefersConfidentiality=true` and caller not authorised) | pgr-services masker | Product | MOZ\_015 \+ clarification per ADR-008 |
...
| 40 | `LanguageStrategy` consumer in pgr-services (separate from novu-bridge) for SMS template resolution | pgr-services | Product | needed for pt\_MZ SMS body fallback |
...
| §1.4 SMS notifications per status change | ADR-002 \+ ADR-003 \+ ADR-006 \+ MOZ\_007 | planned |
...
| §5.2 Case Management Portal filters | MOZ\_017 / MOZ\_018 | planned |
...
| §5.3 Supervisory/Executive Dashboard | MOZ\_024 / row 31 | planned |
...
| MOZ\_\* roadmap items | `docs/superpowers/specs/2026-06-18-mozambique-engineering-tracker-scoping.md` |
...
| `RAINMAKER-PGR.IgeComplaintExtendedAttributes` | NEW JSON Schema | (validates extended\_attributes; no data entries — it's a validator schema) |
...
| `LanguageStrategy` (mz → pt\_MZ) | Not started | Hari |
| `TemplateBinding` SMS templates (en\_IN) | Drafted (BRD §5.1.E text) | Hari |
| `TemplateBinding` SMS templates (pt\_MZ) | Translation pending | Translator |
...
| MOZ\_007, MOZ\_008, … | Roadmap item identifiers per the Mozambique engineering tracker |
...
| pt\_MZ | Portuguese (Mozambique) locale |
