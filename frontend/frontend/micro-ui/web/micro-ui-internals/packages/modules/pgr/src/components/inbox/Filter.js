...
const Filter = (props) => {
  ...
  useEffect(() => {
    ...
    for (const property in pgrfilters) {
      if (Array.isArray(pgrfilters[property])) {
        ...
        const params = pgrfilters[property].map((prop) => prop.code).join();
        if (params) {
    ...
    for (const property in wfFilters) {
      if (Array.isArray(wfFilters[property])) {
        const params = wfFilters[property].map((prop) => prop.code).join();
        if (params) {
  ...
  const ifExists = (list, key) => list.filter((object) => object.code === key.code).length;
  ...
  const onRemove = (index, key) => {
    const afterRemove = pgrfilters[key].filter((_, i) => i !== index);
  ...
  function clearAll() {
    const pgrReset = { serviceCode: [], locality: [], applicationStatus: [] };
    const wfReset = { assignee: [{ code: [] }] };
    setPgrFilters(pgrReset);
    setWfFilters(wfReset);
    pgrQuery = {};
    wfQuery = {};
    setSelectedAssigned("");
  ...
  const GetSelectOptions = (label, options, selected, select, optionKey, onRemoveFn, key) => {
        ...
        <div className="tag-container">
          {pgrfilters[key].length > 0 &&
            pgrfilters[key].map((value, index) => (
              <RemoveableTag key={index} text={`${value[optionKey].slice(0, 22)} ...`} onClick={() => onRemoveFn(index, key)} />
            ))}
        </div>
      </div>
    );
      ...
      <div className="filter">
        <div className="filter-card">
          ...
          <div>
            ...
            <div>
              {GetSelectOptions(t("CS_COMPLAINT_DETAILS_COMPLAINT_SUBTYPE"), serviceDefs, selectedComplaintType, complaintType, "i18nKey", onRemove, "serviceCode")}
            </div>
            <div>
