...
> **`<tenant>` in the snippets below is a placeholder for your SSH host name.** Either set up an SSH config alias matching your inventory hostname (`Host mytenant` → `HostName <ansible_host>`), or substitute `root@<ansible_host>` from your tenant's `host_vars/*.yml`.
...
```bash
ssh -L 18200:127.0.0.1:18200 <tenant>
# in another terminal:
...
```bash
ssh <tenant> "cat /opt/digit/.openbao/init.json" | python3 -c "import json,sys; d=json.load(sys.stdin); print('root token:', d['root_token']); print('unseal key:', d['keys_base64'][0])"
```
...
export BAO_TOKEN=$(ssh <tenant> "jq -r .root_token /opt/digit/.openbao/init.json")
bao kv get kv/digit/<tenant>
# or via curl:
...
```bash
bao kv put kv/digit/<tenant> postgres_password='new-strong-pwd'
# or merge with existing:
bao kv patch kv/digit/<tenant> keycloak_admin_password='another-pwd'
```
...
```bash
ssh <tenant> "tail -f /var/lib/docker/volumes/digit_openbao_data/_data/audit.log"
```
