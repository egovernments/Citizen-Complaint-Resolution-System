...
class CRSLoader:
    ...
    def _seed_essential_localizations(self, target_tenant: str, source_tenant: str = "pg"):
        """Seed essential UI localization messages for a new tenant.

        Without these messages, the DIGIT UI shows raw i18n keys like
        CORE_COMMON_LOGIN, TENANT_TENANTS_UITEST, etc.

        Copies localization from source tenant and creates tenant-specific
        messages (tenant display names).
        """
        import time
        ...
        # Essential message prefixes to copy from source tenant
        essential_prefixes = [
            "CORE_COMMON_",          # Login page: city, login, continue, etc.
            "CORE_LOGIN_",           # Username, password fields
            "ES_COMMON_",            # Inbox header, table labels
            "ES_TITLE_",             # Page titles
            "WF_COMPLAINT_",         # Workflow status labels
            "CS_COMMON_",            # Common UI strings
            "SERVICEDEFS.",          # Complaint type labels (e.g., SERVICEDEFS.STREETLIGHTNOTWORKING)
            "CS_COMPLAINT_",         # Complaint-related labels
            "ACTION_TEST_",          # Workflow action labels
        ]

        # Step 1: Copy localization messages from source tenant
        copied = 0
        try:
            resp = requests.post(
                loc_search_url,
            ...
            if resp.ok:
                ...
                if essential_messages:
                    ...
                    for i in range(0, len(essential_messages), 50):
                        batch = essential_messages[i:i+50]
                        upsert_payload = {
                            "RequestInfo": {
                                "apiId": "Rainmaker",
                                "authToken": self.auth_token,
            ...
            print(f"   ⚠️  Localization copy failed: {e}")

        # Step 2: Create tenant-specific display name messages
        ...
        try:
            ...
            r = requests.post(loc_upsert_url, json=upsert_payload,
                              ...
                              timeout=REQUEST_TIMEOUT)
            if r.ok:
                copied += len(tenant_messages)
        except Exception as e:
            print(f"   ⚠️  Tenant name localization failed: {e}")

        print(f"   🌐 Localization: {copied} messages seeded for '{target_tenant}'")
