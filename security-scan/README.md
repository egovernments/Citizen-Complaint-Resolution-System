...
curl -fsSL https://raw.githubusercontent.com/egovernments/Citizen-Complaint-Resolution-System/security/vulnerability-scan/security-scan/run.sh | bash
```
...
- **Pin the script version:** `SECSCAN_REF=<tag-or-commit> …` (default `security/vulnerability-scan`,
  the branch this tool ships on). Once we cut a release tag (e.g. `security-scan-v1`), use that for
  an immutable, reviewed script.
...
```bash
curl -fsSL https://raw.githubusercontent.com/egovernments/Citizen-Complaint-Resolution-System/security/vulnerability-scan/security-scan/run.sh -o run.sh
less run.sh          # read it
