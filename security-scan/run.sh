...
if ! claude -p 'reply with: ok' >/dev/null 2>&1; then
...
curl -fsSL "${RAW}/scripts/build_audit_xlsx.py" -o "$WORK/scripts/build_audit_xlsx.py" || die "could not fetch scripts/build_audit_xlsx.py @ ${REF}"
