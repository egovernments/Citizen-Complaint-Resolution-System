...
**Status:** parked as a known, accepted k3s-only gap (5 dashboard tests). Fully root-caused below; not fixable via CCRS chart/env/config — needs a gateway code fix + image rebuild, **carried in our own build (kept internal — not filed upstream).** Compose/bomet unaffected. Revisit when we ship a patched `egovio/gateway` image.
...
**Source located (checked 2026-07):** `egovernments/Digit-Core` → `core-services/gateway/src/main/java/com/example/gateway/filters/pre/CorrelationIdFilter.java` (`/filestore` skip + content-type dispatch) and `.../helpers/CorrelationIdFilterHelper.java:77` (`return Mono.just(body);`) — our decompiled line number matched the source exactly, and HEAD is still unfixed. No public issue describes the general bodyless-GET NPE.
**Disclosure: KEEP INTERNAL — do NOT file upstream (team decision, 2026-07).** Carry the fix in **our own** gateway build: patch `CorrelationIdFilterHelper` (`Mono.justOrEmpty(body)`) / generalize the `/filestore` skip to bodyless requests, rebuild `egovio/gateway`, and pin that image on k3s. Do not open a `egovernments/Digit-Core` issue/PR. Revisit the disclosure decision later if we choose to.
