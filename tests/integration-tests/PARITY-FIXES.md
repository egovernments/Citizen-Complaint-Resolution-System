...
## §1.10 · Configurator DSS / PGR dashboard doesn't render on **k3s**  🔍 Open (root-caused, fix TBD)
**Symptom (k3s-only):** 5 admin tests — `overview card shows 3 KPI metrics`, `KPI values show numbers`, `all chart canvases render`, `chart section titles are visible`, `breakdown table with 4 tabs` — pass on Compose **and** bomet but fail on k3s. Not stale data: a k3s admin re-run *with PGR complaints present* (post-§1.9) reproduced the failures byte-identically (40/22/51 unchanged).
**How it surfaced:** invisible until §1.9. The complaint pipeline had to work before the dashboard tests could reach the chart assertions; once Compose could create complaints, its dashboard rendered and the k3s divergence became visible.
**Where:** the dashboard is a **configurator SPA route** — `${CONFIGURATOR_BASE}/manage/pgr-dashboard` (Chart.js, `tests/admin/pgr-dashboard.spec.ts`). On k3s the page's own scaffolding is **absent** — `DSS Overview` / `Total Complaints` / `Status by Tenant` titles and the `<canvas>` elements never appear (a no-data dashboard would still render its shell with zeros). Other configurator-manage routes DO pass on k3s (§2.6), so the SPA itself loads — only this route's data-fed charts don't.
**Root cause (probable, unconfirmed):** the configurator deployed live-only on k3s (§1.2) is a different/older build than Compose's, or the dashboard's aggregate data fetch (PGR counts) isn't routed through the k3s Spring gateway the way Kong routes it on Compose. Needs a browser-console/network trace on the k3s `/manage/pgr-dashboard` to confirm which.
**Next step:** diff the k3s vs Compose configurator image/build; capture the failing XHR on the k3s dashboard. Then either pin the same configurator build on k3s or add the missing gateway route. (This is the mirror image of §1.9 — there Compose was behind; here k3s is.)
