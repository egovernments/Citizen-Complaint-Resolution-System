/**
 * New Citizen SSO Provisioning
 *
 * Simulates the EXACT flow of a new user logging in via Google SSO:
 * 1. Create a fresh KC user via KC admin port (simulates first-time Google SSO)
 * 2. Get a JWT for that user via ROPC against the KC realm
 * 3. Make API calls through the proxy with that JWT
 * 4. Verify the proxy provisions a DIGIT user and proxies successfully
 *
 * This spec catches the CITIZEN OTP auth bug and any other provisioning issues.
 * It must NOT use the ADMIN user — it must create a brand new CITIZEN.
 *
 * @local-only: Requires Keycloak admin port (18180) which is only available
 * in a local stack (LOCAL_STACK=1). Excluded from chromium runs on deployed
 * environments via grepInvert: EXCLUDE_LOCAL_ONLY.
 *
 * NOTE: This spec is destructive — it creates (and cleans up) a temporary KC
 * user in the realm. Run only against a local/fresh KC instance.
 *
...
test.describe('New Citizen Proxy Flow (simulates Google SSO)', () => {
  ...
  test.beforeAll(async () => {
    // Check KC availability first — skip entire suite if not reachable
    try {
      const r = await fetch(`${KC_INTERNAL}/realms/master`, { signal: AbortSignal.timeout(2000) });
      kcAvailable = r.ok;
    } catch { /* not reachable */ }
    if (!kcAvailable) return; // skip setup, tests will skip via beforeEach

    kcAdminToken = await getKcAdminToken();

    // Create a fresh user in KC (simulates what Google SSO does)
    const createResp = await fetch(`${KC_INTERNAL}/admin/realms/${KC_REALM}/users`, {
      method: 'POST',
      headers: {
    ...
    const usersResp = await fetch(
      `${KC_INTERNAL}/admin/realms/${KC_REALM}/users?email=${encodeURIComponent(TEST_EMAIL)}&exact=true`,
  ...
  test.afterAll(async () => {
    ...
    if (kcUserId && kcAdminToken) {
      await fetch(`${KC_INTERNAL}/admin/realms/${KC_REALM}/users/${kcUserId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${kcAdminToken}` },
      });
    }
  ...
  test('MDMS search works for new citizen through proxy', {
    tag: ['@local-only', '@area:keycloak', '@persona:citizen'],
  }, async ({ page }) => {
  ...
  test('localization search works for new citizen through proxy', {
    tag: ['@local-only', '@area:keycloak', '@persona:citizen'],
  }, async ({ page }) => {
    await page.goto(`${BASE_URL}/digit-ui/citizen`, {
      waitUntil: 'domcontentloaded',
  ...
  test('access control works for new citizen through proxy', {
    tag: ['@local-only', '@area:keycloak', '@persona:citizen'],
  }, async ({ page }) => {
    await page.goto(`${BASE_URL}/digit-ui/citizen`, {
      waitUntil: 'domcontentloaded',
  ...
  test('PGR complaint creation works for new citizen through proxy', {
    tag: ['@local-only', '@area:keycloak', '@persona:citizen'],
  }, async ({ page }) => {
