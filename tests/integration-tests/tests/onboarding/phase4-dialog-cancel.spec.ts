...
async function writeEmployeesFixture(file: string, ids: Ids): Promise<void> {
  const wb = new ExcelJS.Workbook();
...
test.describe('Onboarding — Phase 4 confirm dialog', () => {
  ...
  test('Cancel on the confirm dialog dismisses without firing creates', { tag: ['@area:onboarding', '@kind:regression', '@layer:ui', '@persona:admin'] }, async ({ page }) => {
    ...
    const tenantFixture = path.join(os.tmpdir(), `tenant-p4cancel-${ids.SUFFIX}.xlsx`);
    const boundaryFixture = path.join(os.tmpdir(), `boundary-p4cancel-${ids.SUFFIX}.xlsx`);
    const mastersFixture = path.join(os.tmpdir(), `masters-p4cancel-${ids.SUFFIX}.xlsx`);
    const employeesFixture = path.join(os.tmpdir(), `employees-p4cancel-${ids.SUFFIX}.xlsx`);
    ...
    await writeEmployeesFixture(employeesFixture, ids);
