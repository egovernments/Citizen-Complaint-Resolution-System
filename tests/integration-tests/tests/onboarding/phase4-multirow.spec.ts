...
async function writeMixedEmployeesFixture(file: string, ids: Ids): Promise<void> {
  const wb = new ExcelJS.Workbook();
  ...
  // Three valid rows. Use unique 9-digit mobile numbers (Kenya format).
  for (let i = 1; i <= 3; i++) {
    sheet.addRow({
      employeeCode: `PWE${i}_${ids.SUFFIX}`,
      name: `Valid ${i} ${ids.SUFFIX}`,
      mobileNumber: `7${ids.SUFFIX.slice(-7)}${i}`.slice(0, 9),
      dob: '1990-01-01',
  ...
  sheet.addRow({
    ...
    mobileNumber: `7${ids.SUFFIX.slice(-7)}4`.slice(0, 9), dob: '1990-01-01',
    department: ids.GHOST_DEPT, designation: ids.DESIG_CODE,
  ...
  sheet.addRow({
    ...
    mobileNumber: `7${ids.SUFFIX.slice(-7)}5`.slice(0, 9), dob: '1990-01-01',
    department: ids.DEPT_CODE, designation: ids.DESIG_CODE,
    roles: ids.GHOST_ROLE, jurisdictions: ids.BOUNDARY_ROOT, dateOfAppointment: '2024-01-01',
  });
  await wb.xlsx.writeFile(file);
}
...
test.describe('Onboarding — Phase 4 multi-row mixed', () => {
  ...
  test('3 valid + 2 invalid → preview reports 5 total / 3 valid / 2 errors and Create button reads 3', { tag: ['@area:onboarding', '@kind:regression', '@layer:ui', '@persona:admin'] }, async ({ page }) => {
    ...
    const tenantFixture = path.join(os.tmpdir(), `tenant-p4mr-${ids.SUFFIX}.xlsx`);
    const boundaryFixture = path.join(os.tmpdir(), `boundary-p4mr-${ids.SUFFIX}.xlsx`);
    const mastersFixture = path.join(os.tmpdir(), `masters-p4mr-${ids.SUFFIX}.xlsx`);
    const employeesFixture = path.join(os.tmpdir(), `employees-p4mr-${ids.SUFFIX}.xlsx`);
    ...
    await writeMixedEmployeesFixture(employeesFixture, ids);
