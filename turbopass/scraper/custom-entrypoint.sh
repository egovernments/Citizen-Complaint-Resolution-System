		...
		if [[ $CURL_STATUS_CODE = "200" || $CURL_STATUS_CODE = "000" ]]; then
			(
				if [[ -n "${OVERPASS_PLANET_PREPROCESS:+x}" ]]; then
					...
					/app/bin/init_osm3s.sh /db/planet.osm.bz2 /db/db /app "${META}" "--version=2026-06-09T20:21:43Z --compression-method=${OVERPASS_COMPRESSION} --map-compression-method=${OVERPASS_COMPRESSION} --flush-size=${OVERPASS_FLUSH_SIZE}" &&
