...
class UnifiedExcelReader:
    ...
    def read_complaint_types(self, tenant_id: str, dept_name_to_code: dict = None,
        ...
        a complaint (verbatim). 'menuPath'/'menuPathName' are NOT emitted — grouping is derived from
        parentCode/path.
...
class APIUploader:
    ...
    def upload_file_to_filestore(self, file_path: str, tenant_id: str, module: str = "HCM-ADMIN-CONSOLE") -> str:
        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                if not response.ok:
                    print(f"❌ Filestore upload failed ({response.status_code}): {response.text[:300]}")
                    return None
                response.raise_for_status()
    ...
    # ========================================================================
    # HIERARCHY SCHEMA UPDATE
    # ========================================================================

    def update_hierarchy_schema(self, tenant_id: str, hierarchy_type: str,
                                 highest_hierarchy: str, lowest_hierarchy: str) -> dict:
        """Update CMS-BOUNDARY.HierarchySchema MDMS records with the given hierarchy details.

        Called automatically after boundary hierarchy creation/selection so that the
        CMS and HRMS module schema records always reflect the correct hierarchy type
        and the actual top/bottom boundary levels.

        Args:
            tenant_id:          Tenant to update records for.
            hierarchy_type:     Hierarchy type string (e.g. "ADMIN").
            highest_hierarchy:  Name of the top boundary level (first level created).
            lowest_hierarchy:   Name of the bottom boundary level (last level created).

        Returns:
            dict with keys: updated (int), failed (int), errors (list).
        """
        ...
        print(f"   Highest level   : {highest_hierarchy}")
        print(f"   Lowest level    : {lowest_hierarchy}")
        print("="*60)
        ...
        try:
            ...
            msg = f"Search failed: {str(e)[:200]}"
            print(f"   ❌ {msg}")
            result["errors"].append(msg)
            return result

        if not records:
            print(f"   ℹ️  No {schema_code} records found — creating default CMS record ...")
            create_url = f"{self.mdms_url}/v2/_create/{schema_code}"
            ...
            for module in default_modules:
                ...
                try:
                    cr = requests.post(create_url, json=create_payload,
                                       headers={"Content-Type": "application/json"})
                    print(f"   ↩️  Response   : {cr.status_code}")
                    if cr.status_code in (200, 201):
                        print(f"   ✅ Created HierarchySchema for module '{module}'")
                        result["updated"] += 1
                    else:
                        err = cr.text[:500]
                        print(f"   ❌ Failed to create '{module}': {err}")
                        result["failed"] += 1
                        result["errors"].append({"module": module, "error": err})
                except Exception as e:
                    msg = str(e)[:200]
                    print(f"   ❌ Error creating '{module}': {msg}")
                    result["failed"] += 1
                    result["errors"].append({"module": module, "error": msg})
            return result

        print(f"   Found {len(records)} record(s)")

        # ── Step 2: update each record in-place ──────────────────────────────
        for record in records:
            data = record.get("data", {})
            module = data.get("moduleName", record.get("uniqueIdentifier", "?"))

            data["hierarchy"]        = hierarchy_type
            data["highestHierarchy"] = highest_hierarchy
            data["lowestHierarchy"]  = lowest_hierarchy
            ...
            update_payload = {
                ...
                "Mdms": {
                    "tenantId":          tenant_id,
                    "schemaCode":        schema_code,
                    "uniqueIdentifier":  record.get("uniqueIdentifier"),
                    "id":                record.get("id"),
                    "data":              data,
                    "auditDetails":      record.get("auditDetails"),
            ...
            try:
                upd = requests.post(update_url, json=update_payload,
                                     headers={"Content-Type": "application/json"})
                if upd.status_code == 200:
                    ...
                    err = upd.text[:200]
                    print(f"   ❌ Failed module '{module}': {err}")
                    result["failed"] += 1
                    result["errors"].append({"module": module, "error": err})
            except Exception as e:
                msg = str(e)[:200]
                print(f"   ❌ Error module '{module}': {msg}")
                result["failed"] += 1
                result["errors"].append({"module": module, "error": msg})
